...
class AgentNotifyCliTest(unittest.TestCase):
    ...
    def test_macos_notifier_helper_handles_click_with_detail_card(self):
        ...
        self.assertIn('NSButton(title: "关闭"', source)
        self.assertIn("closeButton(target: self)", source)
        self.assertIn("#selector(AppDelegate.closeDetailCard)", source)
        self.assertIn("button.bezelStyle = NSButton.BezelStyle.rounded", source)
        self.assertIn("button.keyEquivalent = \"\\r\"", source)
        self.assertIn("closeButton.trailingAnchor.constraint(equalTo: cardSurface.trailingAnchor", source)
        self.assertIn("closeButton.bottomAnchor.constraint(equalTo: cardSurface.bottomAnchor", source)
