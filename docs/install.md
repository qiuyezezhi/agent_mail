...
- macOS 通知优先使用隐藏后台 helper app，位置是 `~/Library/Application Support/agent-notify/notifier/agent-notify.app`。它不会出现在启动台、Dock 或 `/Applications`；点击通知后会弹出临时详情卡片，显示完整消息内容和读取命令；如果无法编译 helper app，会回退到 `osascript`
